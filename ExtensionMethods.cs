...
namespace CatBot
{
    internal static class ExtensionMethods
    {
        ...
        internal static async Task<bool> TryRespondAsync(this SnowflakeObject obj, params object[] parameters)
        {
            ...
            {
                if (parameters[0] is DiscordInteractionResponseType || parameters[0] is int)
                    await interaction.CreateResponseAsync((DiscordInteractionResponseType)parameters[0], (DiscordInteractionResponseBuilder)(parameters.Length == 2 ? parameters[1] : null));
                else if (parameters[0] is DiscordInteractionResponseBuilder)
                    await interaction.CreateResponseAsync(DiscordInteractionResponseType.ChannelMessageWithSource, (DiscordInteractionResponseBuilder)parameters[0]);
                else if (parameters[0] is string)
                {
                    if (parameters.Length >= 2 && (parameters[1] is DiscordEmbed || parameters[1] is DiscordEmbedBuilder))
                    {
                        await interaction.CreateResponseAsync(DiscordInteractionResponseType.ChannelMessageWithSource, new DiscordInteractionResponseBuilder().WithContent((string)parameters[0]).AddEmbed((DiscordEmbed)parameters[1]).AsEphemeral((bool)(parameters.Length > 2 ? parameters[2] : false)));
                    }
                    else
                    {
                        await interaction.CreateResponseAsync(DiscordInteractionResponseType.ChannelMessageWithSource, new DiscordInteractionResponseBuilder().WithContent((string)parameters[0]).AsEphemeral((bool)(parameters.Length == 2 ? parameters[1] : false)));
                    }
                }
                else if (parameters[0] is DiscordEmbed || parameters[0] is DiscordEmbedBuilder)
                {
                    await interaction.CreateResponseAsync(DiscordInteractionResponseType.ChannelMessageWithSource, new DiscordInteractionResponseBuilder().AddEmbed((DiscordEmbed)parameters[0]).AsEphemeral((bool)(parameters.Length == 2 ? parameters[1] : false)));
                }
            }
            else
                result = false;
