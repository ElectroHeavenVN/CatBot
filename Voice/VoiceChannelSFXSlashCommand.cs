...
namespace DiscordBot.Voice
{
    ...
    {
        [SlashCommand("speakfile", "Chọn file để nói")]
        public async Task Speak(InteractionContext ctx, [Option("file", "Tên file (cách nhau bằng dấu cách)"), Autocomplete(typeof(VoiceSFXChoiceProvider))] string fileNames) => await VoiceChannelSFXCore.Speak(ctx.Interaction, fileNames.Split(' '));

        [SlashCommand("reconnect", "Kết nối lại kênh thoại")]
        ...
        public async Task Delay(InteractionContext ctx, [Option("delay", "giá trị"), Minimum(0), Maximum(5000)] long delay = 250) => await VoiceChannelSFXCore.Delay(ctx.Interaction, (int)delay);
    ...
    internal class VoiceSFXChoiceProvider : IAutocompleteProvider
    {
        public Task<IEnumerable<DiscordAutoCompleteChoice>> Provider(AutocompleteContext ctx)
        {
            ...
            List<FileInfo> sfxFiles = new DirectoryInfo(Config.SFXFolder).GetFiles().Concat(new DirectoryInfo(Config.SFXFolderSecret).GetFiles()).ToList();
